...
class ProductsApplicationTests {
	...
	@Test
	void reserveStockDecrementsStockInBothReplicas() throws Exception {
		JsonNode product = createProduct(adminToken(), "Cadeira", "Cadeira ergonomica", "899.90", 5);
		String productId = product.get("id").asString();

		mockMvc.perform(post("/products/{id}/stock/reserve", productId)
						.header(HttpHeaders.AUTHORIZATION, "Bearer " + userToken())
						.contentType(MediaType.APPLICATION_JSON)
		...
		String primaryContent = Files.readString(productRepository.primaryFile());
		String replicaContent = Files.readString(productRepository.replicaFile());
		assertThat(primaryContent).contains("\"stock\" : 3");
		assertThat(replicaContent).contains("\"stock\" : 3");
	}

	@Test
	void reserveStockRejectsInsufficientStock() throws Exception {
		JsonNode product = createProduct(adminToken(), "Monitor", "Monitor 27", "1299.90", 1);

		mockMvc.perform(post("/products/{id}/stock/reserve", product.get("id").asString())
						.header(HttpHeaders.AUTHORIZATION, "Bearer " + userToken())
						.contentType(MediaType.APPLICATION_JSON)
						.content(objectMapper.writeValueAsString(Map.of("quantity", 2))))
